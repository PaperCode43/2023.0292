...
function [Y_true,dY_true,f_enhanced,df_enhanced,f_SK,df_SK]=KrigEuro_cokriging_vega(S0,r,X,T,K,M,t,poly_d,Tru_MC_idx)
...
%X=[97;101;103];
%X=[0.01;0.05;0.10;0.16;0.22;0.29];
Xtest=(0.01:0.01:0.3)';
%Xtest=(95:0.5:105)';
%Xtest=(98:0.5:102)';
n=length(X);
if Tru_MC_idx==1
    [Y,dY]=bls_Euro_call_vega(S0,K,r,X,t,T);
    vY=0.0*ones(n,1);
    vdY=0.0*ones(n,1);
    vMatrix = 0.0*diag(ones(2*n,1));%%%make sure the inverse exists
else
    [Y,vY,dY,vdY,vYdY]=EuroMC_vega(S0,r,X,T,K,M);
    vMatrix = [diag(vY),diag(vYdY);diag(vYdY),diag(vdY)];
end

%[Yt,dYt]=bls_Euro_call_vega(S0,K,r,X,t,T);
B1=polybasis(X,poly_d);
gammaP=2;
SKmodel1=SKfit(X, Y, B1, vY, gammaP,0.01);
SKmodel1_der=SKfit_der(X, dY, B1, vdY, gammaP,0.01);
SKmodel_enhanced=SK_grad1_fit(X, Y, dY, B1, vY, vMatrix, gammaP,0.01);

[Y_true,dY_true]=bls_Euro_call_vega(S0,K,r,Xtest,t,T);
...
% legend('Selected point','True surface','SK','ESK')
% subplot(1,2,2)
% plot(X,dY,'kx',Xtest,dY_true,'r',Xtest,df_SK,'c--',Xtest,df_enhanced,'b--','LineWidth',2)
% legend('Selected point','True surface','D-SK','ESK-D')
...
function [V_c,dV_c]=bls_Euro_call_vega(S,K,r,sigma,t,T)
tau=T-t;
d1=1./(sigma*sqrt(tau)).*(log(S/K)+(r+sigma.^2/2)*tau);
d2=d1-sigma*sqrt(tau);
V_c = normcdf(d1,0,1).*S-normcdf(d2,0,1).*K*exp(-r*tau);
dV_c= S*sqrt(T)*normpdf(d1,0,1);





function [V,vV,G,vG,vVG]=EuroMC_vega(S0,r,sigma,T,K,M)
%M = ones(size(sigma))*M;%% larger sigma has larger variance
M=round(sigma*10*M);
l=length(sigma);
V=zeros(l,1);vV=zeros(l,1);G=zeros(l,1);vG=zeros(l,1);vVG=zeros(l,1);
for i=1:l
Z=normrnd(0,1,M(i),1);
ST=S0*exp((r-0.5*sigma(i)^2)*T+sigma(i)*sqrt(T)*Z);
Payoff=exp(-r*T)*max(ST-K,0);
V(i)=mean(Payoff);
vV(i)=var(Payoff)/M(i);
IPA_sigma=exp(-r*T)*ST.*(-sigma(i)*T+sqrt(T)*Z).*(ST>K);
G(i)=mean(IPA_sigma);
vG(i)=var(IPA_sigma)/M(i);
covVG = cov(IPA_sigma,Payoff);
vVG(i) = covVG(1,2)/M(i);
end

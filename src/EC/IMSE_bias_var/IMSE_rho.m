% r test [0,0.1]
function [IMSE_ESK_Y,IMSE_ESK_dY,IMSE_SK_Y,IMSE_SK_dY]=IMSE_rho(S0,sigma,T,K,M,t,poly_d,rep)
u=lhsdesign(6,rep);
Y_true=zeros(101,rep);%%81 test points 
dY_true=zeros(101,rep);f_enhanced=zeros(101,rep);df_enhanced=zeros(101,rep);
f_SK=zeros(101,rep);df_SK=zeros(101,rep);
...
[Y_true(:,i),dY_true(:,i),f_enhanced(:,i),df_enhanced(:,i),f_SK(:,i),df_SK(:,i)]...
    =KrigEuro_cokriging_rho(S0,X,sigma,T,K,M,t,poly_d,0);

function model = SK_grad1_fit_allgreek(X, Y, dY, B, Vhat, Vmatrix, gammaP,lambda)
...
[parms,~,exitflag,~] = fmincon(@(x) neg_LL_p_parms(x,k,d,D,B,V,Y,gammaP,lambda),...
...
C00 = tau2hat*Rhat;
C01=zeros(k,k);C02=zeros(k,k);C03=zeros(k,k);C04=zeros(k,k);
C11=zeros(k,k);C12=zeros(k,k);C13=zeros(k,k);C14=zeros(k,k);
C22=zeros(k,k);C23=zeros(k,k);C24=zeros(k,k);C33=zeros(k,k);C34=zeros(k,k);C44=zeros(k,k);
...
for i=1:k
    for h=1:k
        ...
        C04(i,h) = 2*thetahat(4)*(X(i,4)-X(h,4))*C00(i,h);
        ...
        C14(i,h) = -2*thetahat(1)*(X(i,1)-X(h,1))*2*thetahat(4)*(X(i,4)-X(h,4))*C00(i,h);
        ...
        C24(i,h) = -2*thetahat(2)*(X(i,2)-X(h,2))*2*thetahat(4)*(X(i,4)-X(h,4))*C00(i,h);
        C33(i,h) = 2*thetahat(3)*(1 - 2*thetahat(3)*(X(i,3)-X(h,3))^2)*C00(i,h);
        C34(i,h) = -2*thetahat(3)*(X(i,3)-X(h,3))*2*thetahat(4)*(X(i,4)-X(h,4))*C00(i,h);
        C44(i,h) = 2*thetahat(4)*(1 - 2*thetahat(4)*(X(i,4)-X(h,4))^2)*C00(i,h);
    end
end
C10=-C01;C20=-C02;C30=-C03;C40=-C04;
C21=C12;C31=C13;C32=C23;C41=C14;
C42=C24;C43=C34;
Sigma_M=[C00,C01,C02,C03,C04;C10,C11,C12,C13,C14;C20,C21,C22,C23,C24;C30,C31,C32,C33,C34;C40,C41,C42,C43,C44];

% S0 test [80,120]
function [IMSE_ESK_Y,IMSE_ESK_dY,IMSE_SK_Y,IMSE_SK_dY]=IMSE_vega(S0,r,T,K,M,t,poly_d,rep)
sd=30;%%50 test points
u=lhsdesign(5,rep);
Y_true=zeros(sd,rep); 
dY_true=zeros(sd,rep);f_enhanced=zeros(sd,rep);df_enhanced=zeros(sd,rep);
f_SK=zeros(sd,rep);df_SK=zeros(sd,rep);
...
for i=1:rep
    ...
    [Y_true(:,i),dY_true(:,i),f_enhanced(:,i),df_enhanced(:,i),f_SK(:,i),df_SK(:,i)]...
        =KrigEuro_cokriging_vega(S0,r,X,T,K,M,t,poly_d,0);

%% this file to calculate the analytical formula of portfolio value
function [Phi_tau,Phi_tau_delta,Phi_tau_sigma,Phi_tau_theta]=portfolio_basket_price(S_tau,r,sigma,T,K1,K2,K3)
%Rho=rho'*rho;
%Rho(logical(eye(size(Rho))))=1;
%sigma_new=1/2*sqrt(sigma*Rho*sigma');
Nt=size(S_tau,1);
Phi_tau=zeros(Nt,1);
Phi_tau_delta=zeros(Nt,1);
Phi_tau_sigma=zeros(Nt,1);
Phi_tau_theta=zeros(Nt,1);

for i=1:Nt
    %price
    S0=S_tau(i);
    Ttau=T(i);
    [c1,c1_d1,c1_dsigma,c1_dT]=bls_Euro_call(S0,Ttau,r,sigma,K1);
    [c2,c2_d1,c2_dsigma,c2_dT]=bls_Euro_call(S0,Ttau,r,sigma,K2);
    [c3,c3_d1,c3_dsigma,c3_dT]=bls_Euro_call(S0,Ttau,r,sigma,K3);
    Phi_tau(i)=-2*c2+c1+c3;
    %delta
    Phi_tau_delta(i)=-2*c2_d1+c1_d1+c3_d1;
    % sigma
    Phi_tau_sigma(i)=-2*c2_dsigma+c1_dsigma+c3_dsigma;
    % Theta
    Phi_tau_theta(i)=-2*c2_dT + c1_dT+c3_dT;
end





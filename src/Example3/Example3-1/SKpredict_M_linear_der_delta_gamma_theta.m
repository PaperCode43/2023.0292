...
function [w_delta,w_gamma,w_theta,beta] = SKpredict_M_linear_der_delta_gamma_theta(model,Xpred,Bpred)
...
if gammaP == 3
    ...
    Rpredl = tau2*prod(((D<=(T./2)).*(1-6*(D./T).^2+6*(D./T).^3) ...
        +((T./2)<D & D<=T).*(2*(1-D./T).^3)),2);
else
    Rpredl = tau2*exp(sum(-D.*repmat(reshape(theta,[1 d]),[k 1 K]),2));
    pRpred=D(:,1,:);  
    Rpred_delta = 2 * theta(1) * (1 - 2 * theta(1) * pRpred).*Rpredl;
    Rpred_theta = 2 * theta(2) * (1 - 2 * theta(2) * pRpred).*Rpredl;
    Rpred_gamma = 12*theta(1)^2*Rpredl - 48*theta(1)^3*pRpred.*Rpredl...
         + 16*theta(1)^4*pRpred.^2.*Rpredl;
end
Rpred_delta = reshape(Rpred_delta,[k K 1]);

% calculate responses at prediction points 
%f = Bpred*beta + Rpred'/Sigma*Z;
%e1=ones(k,1);
%w=((Rpred + e1 * (1-e1'/(Sigma)*Rpred)/(e1'/(Sigma)*e1))'/(Sigma))';
w_delta=(Rpred_delta'/(Sigma))';
w_theta=(Rpred_theta'/(Sigma))';
w_gamma=(Rpred_gamma'/(Sigma))';

function model = SKfit(X, Y, B, Vhat, gammaP,lambda)
...
V = diag(Vhat);%to avoid ill-conditioned
scaling = 1;
...
% %parms = ga(@(x) neg_LL_p_parms(x,k,d,D,B,V,Y,gammaP,lambda), 1+d, [],[],[],[],lb,ub,[],[],options);
% parms = parms';  
% 
% 

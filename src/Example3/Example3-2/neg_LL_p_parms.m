function [f,neg_gradient] = neg_LL_p_parms(parms,k,d,D,B,V,Y,gammaP,lambda)
...
if(parms(1) < 0.001 || min(parms(2:d+1)) < 0.0001)
    f = inf;
...
Sigma  = tausq*R + V + 0.0005*eye(size(V));
%Sigma  = tausq*R + V +lambda*eye(size(V));
...
pars=[tausq;theta];
f = 0.5*k*log(2*pi) + log(det(L)) + 0.5*Z'*Z+lambda*norm(pars);
...
if nargout > 1 
    ...
    beta_par_tau2 = (F'/(A)*F) \ F'*((A\C)/(A)) * (F/(F'/(A)*F)*F'*((A)\Y) - Y);    
    beta_par_theta = tau2* ((F'/(A)*F) \ F'*(inv(A)*C1*inv(A))) * (F/(F'*inv(A)*F)*F'*inv(A)*Y - Y);

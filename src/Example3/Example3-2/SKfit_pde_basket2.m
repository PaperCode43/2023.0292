function model = SKfit_pde_basket2(X, Y, B, Vhat, gammaP,lambda,Xtest,r,sigma,Rho,dS1,dS2,Vmatrix)
...
%     end
    ...
    myopt2 = optimset('MaxFunEvals',1000000,'MaxIter',500000);
    par_opt=fmincon(@(x) obj2_whole_3D(x,X,Xtest,Vhat,D,gammaP,Yp,betahat,tau2hat,r,sigma,Rho,T,m,J,Vmatrix),...
...
%function objstage2=obj2(par,X,Xtest,Vhat,D0,gammaP,a,Y,beta)
function objstage2=obj2_nonoise(par,X,Xtest,Vhat,D0,gammaP,Y,beta,tau2,r,sigma)
theta=par;
...
for i=1:k %Xpred - K x d
    R1(i,:) = -2*theta(1)*(Xtest(:,1)' - X(i,1)).* R0(i,:);
    R2(i,:) = -2*theta(2)*(Xtest(:,2)' - X(i,2)).* R0(i,:);
    R11(i,:) = 2*theta(1)*(2*theta(1)*(Xtest(:,1)' - X(i,1)).^2-1).* R0(i,:);
    %R(i,:) = 2*theta(2)*(2*theta(2)*(Xtest(:,2)' - X(i,2)).^2-1).* R0(i,:);
    %R11(i,:) = 12*theta(1)^2*R0(i,:)-48*theta(1)^3*(Xtest(:,1)' - X(i,1)).^2.*R0(i,:)+16*theta(1)^4*(Xtest(:,1)' - X(i,1)).^4.*R0(i,:);
end

%kap=(R1+a*sin(Xtest(:,1)'))';
kap=(R2+r*(ones(k,1)*Xtest(:,1)').*R1+0.5*sigma^2*(ones(k,1)*Xtest(:,1)').^2.*R11-r*R0)';
ob=zeros(K,1);
for i=1:K
    ob(i)=(kap(i,:)/(Sigmahat)*(Y - beta)-r*beta)^2;
    %ob(i)=kap(i,:)/(Sigmahat)*kap(i,:)';
end
objstage2=mean(ob);


% function objstage2=obj2_noise(par,X,Xtest,Vhat,D0,gammaP,b,Y,beta)
% tau2=par(1);
...
% 
% for i=1:k %Xpred - K x d
%      R1(i,:) = 2*theta(1)*(2*theta(1)*(Xtest(:,1)' - X(i,1)).^2-1).* R0(i,:);
%     %R11(i,:) = 12*theta(1)^2*R0(i,:)-48*theta(1)^3*(Xtest(:,1)' - X(i,1)).^2.*R0(i,:)+16*theta(1)^4*(Xtest(:,1)' - X(i,1)).^4.*R0(i,:);
% end
% 
% %kap=(R1+a*sin(Xtest(:,1)'))';

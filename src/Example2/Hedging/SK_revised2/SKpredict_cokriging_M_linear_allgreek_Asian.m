...
function [ws,w_dS0,w_dsigma,w_dtheta,beta] = SKpredict_cokriging_M_linear_allgreek_Asian(model,Xpred,Bpred)
...
R1 = zeros(k,K);R2 = zeros(k,K);R3 = zeros(k,K);
R10 = zeros(k,K);R11 = zeros(k,K);R12 = zeros(k,K);R13 = zeros(k,K);
R20 = zeros(k,K);R21 = zeros(k,K);R22 = zeros(k,K);R23 = zeros(k,K);
R30 = zeros(k,K);R31 = zeros(k,K);R32 = zeros(k,K);R33 = zeros(k,K);


for i=1:k %Xpred - K x d
    R1(i,:) = 2*theta(1)*(Xpred(:,1)' - X(i,1)) .* R0(i,:);%dS0
    R2(i,:) = 2*theta(2)*(Xpred(:,2)' - X(i,2)) .* R0(i,:);%dsigma
    R3(i,:) = 2*theta(3)*(Xpred(:,3)' - X(i,3)) .* R0(i,:);%dtheta
    R10(i,:) = -R1(i,:);
...
Rspred=[R0;R1;R2;R3];
RdS0pred=[R10;R11;R12;R13];
Rdsigmapred=[R20;R21;R22;R23];
Rdthetapred=[R30;R31;R32;R33];
...
ws=((Rspred)'/(Sigmaplus))';
w_dS0=((RdS0pred)'/(Sigmaplus))';%% previous using 1, which is wrong
w_dsigma=((Rdsigmapred)'/(Sigmaplus))';
w_dtheta=((Rdthetapred)'/(Sigmaplus))';
